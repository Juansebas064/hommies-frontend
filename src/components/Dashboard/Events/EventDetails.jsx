...
import { getEventPlace } from "../../../utils/getEventPlace.js";
...
export default function EventDetails({
  ...
  //Descripción del lugar del evento
  const [placeDesc, setPlaceDesc] = useState("Sin descripción");
  //
  const [codigoEvento, setCodigoEvento] = useState(null);
  //
  ...
  useEffect(() => {
    setCodigoEvento(selectedEvent.codigo_evento);
  }, [selectedEvent]);
    ...
    const getEventoData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/evento/${codigoEvento}`
        );
        const lugar = response.data.lugar;
        setLugarEvento(lugar);

        console.log(response)
        const lugarResponse = await axios.get(
          `http://localhost:5000/api/evento/lugar/${lugar}`
        );
        const lugarData = lugarResponse.data;
        setEventPlace(lugarData.nombre)
        setPlaceDesc(lugarData.descripcion)
        console.log(lugarResponse)
      } catch (error) {
  ...
  async function onSubmit(modifiedEventData) {
    try {
      const response = await axios.put(
        `http://localhost:5000/api/evento/editar/:${selectedEvent.codigo_evento}`,
        modifiedEventData,
        {
          headers: {
  ...
  async function deleteEvent() {
    try {
      const response = await axios.post(
        ...
        { codigo_evento: selectedEvent.codigo_evento },
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: localStorage.getItem("token"),
    ...
    <div className="fixed z-[9999] bg-[rgba(0,0,0,0.2)] h-full w-full left-0 top-0 overflow-auto flex justify-center items-center">
      ...
      <div className="z-[9999] bg-white rounded-[14px] relative min-w-[250px] max-w-[370px] sm:max-w-[470px] max-h-[70vh] sm:max-h-[70vh] px-3 sm:px-7 flex-grow overflow-auto">
        ...
        >
          ...
          <textarea
            className={`text-center overflow-y-scroll h-auto col-span-2 mb-3 px-3 cursor-text ${
              modifyingEvent
                ? "border-gray-300 border-[1px] rounded-md"
                : "border-0"
            }`}
          ...
          <p className="text-sm text-center lg:text-center col-span-2">
            <span className="mr-[6px]"> {eventPlace} </span>
          </p>
          <p className="text-sm text-center lg:text-center col-span-2">
            <span className="mr-[6px]">{placeDesc}</span>
          </p>
